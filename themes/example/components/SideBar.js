...
export const SideBar = (props) => {
  ...
  const { latestPosts, tagOptions, notice } = props
  
  return (
    <div className="w-full md:w-64 sticky top-8">
      <aside className="rounded shadow overflow-hidden mb-6">
        <h3 className="text-sm bg-gray-100 text-gray-700 dark:bg-hexo-black-gray dark:text-gray-200 py-3 px-4 dark:border-hexo-black-gray border-b">{locale.COMMON.TAG}</h3>
        <div className="p-4">
          <ul className="list-reset leading-normal">
            {tagptions?.map(tag => (
              <li key={tag.name}>
                <Link href={`/tag/${tag.name}`} passHref>
                  <a className="text-gray-darkest text-sm">#{tag.name}({tag.count})</a>
                </Link>
              </li>
            ))}
          </ul>
